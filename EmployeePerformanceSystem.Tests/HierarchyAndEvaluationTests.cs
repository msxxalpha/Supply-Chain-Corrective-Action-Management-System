...
public class HierarchyAndEvaluationTests
{
    ...
    {
        var ps = new PerformanceService(db);
        var excel = new ExcelService();
        var c = new EvaluationController(db, ps, excel);
        c.ControllerContext = new ControllerContext
        {
            HttpContext = new DefaultHttpContext
            {
                User = new ClaimsPrincipal(new ClaimsIdentity(
                    [new Claim("EmployeeId", employeeId.ToString())], "Test"))
            }
        };
        return c;
    }
    ...
    public async Task Upper_evaluator_sees_complete_subtree()
    {
        ...
        var s = await SeedScenario(db);
        var ps = new PerformanceService(db);

        var bTree = await ps.GetSubordinates(s.b);
        var aTree = await ps.GetSubordinates(s.a);

        Assert.Equal(2, bTree.Count);
        Assert.Contains(bTree, x => x.Id == s.c);
        Assert.Contains(bTree, x => x.Id == s.d);
        Assert.Equal(3, aTree.Count);
        Assert.Contains(aTree, x => x.Id == s.b);
        Assert.Contains(aTree, x => x.Id == s.c);
        Assert.Contains(aTree, x => x.Id == s.d);
    }
    ...
    public async Task Previous_evaluator_scores_are_visible_and_upper_evaluator_can_change_them_with_history()
    {
        ...
        // B (middle evaluator) performs the initial evaluation of C.
        var bController = Controller(db, s.b);
        var first = await bController.Form(new EvaluationController.FormPost
        {
            EmployeeId = s.c,
        ...
        Assert.Equal(s.b, ev.EvaluatorId);
        Assert.Equal(s.b, ev.OriginalEvaluatorId);
        Assert.Equal(26m, ev.Scores.Sum(x => x.Score));

        // A is above B and must be able to open the same evaluation and change it.
        var aController = Controller(db, s.a);
        var second = await aController.Form(new EvaluationController.FormPost
        {
            EmployeeId = s.c,
            PositionId = s.positionId,
        ...
        Assert.Equal(s.a, ev.EvaluatorId);
        Assert.Equal(s.b, ev.OriginalEvaluatorId);
        Assert.Equal(29m, ev.Scores.Sum(x => x.Score));

        var scoreHistory = await db.ScoreHistory.Where(x => x.EvaluationId == ev.Id).OrderBy(x => x.Id).ToListAsync();
        Assert.Equal(2, scoreHistory.Count);
        Assert.Equal(8m, scoreHistory[0].OldScore);
        Assert.Equal(9m, scoreHistory[0].NewScore);
        Assert.Equal(18m, scoreHistory[1].OldScore);
        Assert.Equal(20m, scoreHistory[1].NewScore);
        Assert.Equal(s.a, scoreHistory[0].ChangedBy);

        var evaluatorHistory = await db.EvaluatorHistory.SingleAsync(x => x.EvaluationId == ev.Id);
        Assert.Equal(s.b, evaluatorHistory.PreviousEvaluatorId);
        Assert.Equal(s.a, evaluatorHistory.NewEvaluatorId);
        Assert.Equal(s.a, evaluatorHistory.ChangedBy);
    }
    ...
    public async Task Non_evaluator_cannot_submit_an_evaluation()
    {
        await using var db = CreateDb();
        var s = await SeedScenario(db);
        var cController = Controller(db, s.c);

        var result = await cController.Form(new EvaluationController.FormPost
        {
            EmployeeId = s.d,
            PositionId = s.positionId,
            Scores = new Dictionary<int, decimal> { [s.q1] = 10, [s.q2] = 20 }
        });
    ...
    public async Task Score_above_question_maximum_is_rejected_and_not_saved()
    {
        ...
        var result = await bController.Form(new EvaluationController.FormPost
        {
            EmployeeId = s.c,
            PositionId = s.positionId,
            Scores = new Dictionary<int, decimal> { [s.q1] = 11, [s.q2] = 20 }
        });

        Assert.IsType<ViewResult>(result);
        Assert.Empty(await db.Evaluations.ToListAsync());
        Assert.Empty(await db.ScoreHistory.ToListAsync());
    }
}
...
[Authorize(Policy="AdminOnly")] public class ReportsController(AppDbContext db,ExcelService excel):Controller{
 ...
 async Task<List<Row>> Query(int p){return await db.Evaluations.Where(x=>x.PeriodId==p).Join(db.Employees,ev=>ev.EmployeeId,e=>e.Id,(ev,e)=>new{ev,e}).Join(db.Employees,z=>z.ev.EvaluatorId,a=>a.Id,(z,a)=>new Row(z.e.FullName,a.FullName,z.e.UnitId.ToString(),z.e.PositionId.ToString(),z.ev.Scores.Sum(s=>s.Score),0,z.ev.Status.ToString())).ToListAsync();}

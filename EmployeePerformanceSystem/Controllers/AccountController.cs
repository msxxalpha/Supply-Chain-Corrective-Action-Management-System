...
[HttpPost][ValidateAntiForgeryToken]public async Task<IActionResult> Login(LoginVm m){var u=await db.Users.SingleOrDefaultAsync(x=>x.UserName==m.UserName&&x.IsActive);if(u==null||!PasswordHasher.Verify(m.Password,u.PasswordHash)){ModelState.AddModelError("","نام کاربری یا رمز عبور صحیح نیست.");return View(m);}if(u.EmployeeId.HasValue){var e=await db.Employees.FindAsync(u.EmployeeId.Value);var p=await db.Periods.Where(x=>x.IsOpen).OrderByDescending(x=>x.StartAt).FirstOrDefaultAsync();var now=DateTime.Now;if(e==null||!e.IsEvaluator||p==null||now<p.StartAt||now>p.EndAt.AddDays(1).AddTicks(-1)){ModelState.AddModelError("","ورود ارزیاب در حال حاضر مجاز نیست.");return View(m);}}

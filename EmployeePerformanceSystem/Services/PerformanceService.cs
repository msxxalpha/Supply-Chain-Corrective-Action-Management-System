...
public class PerformanceService(AppDbContext db){
 ...
 public async Task<EvaluationPeriod?> CurrentPeriod(){var now=DateTime.Now;return await db.Periods.Where(x=>x.IsOpen).OrderByDescending(x=>x.StartAt).FirstOrDefaultAsync(x=>x.StartAt<=now&&x.EndAt>=now);}
 ...
 public async Task<bool> CanReviewEvaluation(int actorId,Evaluation ev){if(ev.EvaluatorId==actorId)return true;return await db.Employees.AnyAsync(x=>x.Id==ev.EvaluatorId&&x.SupervisorId==actorId&&x.IsEvaluator&&x.IsActive);}
